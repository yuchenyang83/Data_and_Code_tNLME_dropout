...
#   Date        :    18.08.2026
#   Purpose     :    reproduce the additional nearly Gaussian simulation study
#                    with nu = 100 and 25% dropout across sample sizes
...
#
#   Output .txt files :  Data_and_Code/Data/simulation/SS-simulation-t25-50/SIM1-SIM5/
#
#   R Version   :    R-4.6.0
...
### 25 dropout
alpha = c(-20.44, -0.69, 0.30,  0.30) ## intercept; treat; y_si-1; y_si
### 50 dropout
# alpha = c(-19.09, -0.69, 0.30,  0.30) ## intercept; treat; y_si-1; y_si
### 75 dropout
# alpha = c(-17.85, -0.69, 0.30,  0.30) ## intercept; treat; y_si-1; y_si
...
for (Rep in binrep:Repp) {
  ...
  init.para = list(Beta=matrix(c(fm1.nlme$coefficients$fixed), ncol=1), DD=DD1, sigma=c(fm1.nlme$sigma^2), Phi=para$Phi, nu=50, alpha = alpha1)
  ...
  # est.MNAR.MCEM = try(tNLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                     mechanism='MNAR'), silent = F)
  # if(class(est.MNAR.MCEM) == "try-error") next;
  # 
  # fit.MNAR.MCEM = try(NLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                    mechanism='MNAR'), silent = F)
  ...
  # cat(rep('=', 15), "Student's t of Linear mixed modles with MAR missing", cor.type[1], " errors: ", "\n")
  # init.para$alpha = alpha[-length(alpha)]
  # est.MAR.MCEM = try(tNLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                    mechanism='MAR'), silent = F)
  # if(class(est.MAR.MCEM) == "try-error") next;
  # fit.MAR.MCEM = try(NLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                   mechanism='MAR'), silent = F)
  ...
  # cat(rep('=', 15), "Student's t of Linear mixed modles with MCAR missing", cor.type[1], " errors: ", "\n")
  # init.para$alpha = c(-2.51)
  # est.MCAR.MCEM = try(tNLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                     mechanism='MCAR'), silent = F)
  # if(class(est.MCAR.MCEM) == "try-error") next;
  # fit.MCAR.MCEM = try(NLMM.miss.MCEM(Data, g, init.para, cor.type = c("UNC"), M=1000, M.LL = 1000, P=1, tol = 1e-3, max.iter=max.iter, per=500,
  #                                    mechanism='MCAR'), silent = F)
  ...
  time = c(est.MNAR$model.inf$time, est.MAR$model.inf$time, est.MCAR$model.inf$time, fit.MNAR$model.inf$time, fit.MAR$model.inf$time, fit.MCAR$model.inf$time)
  Qloglik = c(est.MNAR$model.inf$Qloglik, est.MAR$model.inf$Qloglik, est.MCAR$model.inf$Qloglik, fit.MNAR$model.inf$Qloglik, fit.MAR$model.inf$Qloglik, fit.MCAR$model.inf$Qloglik)
  ...
  write(c(Rep, time), paste(PATH1,'time.txt',sep=""), ncol=10000, append=T)
  write(c(Rep, Qloglik), paste(PATH1,'Qloglik.txt',sep=""), ncol=10000, append=T)
