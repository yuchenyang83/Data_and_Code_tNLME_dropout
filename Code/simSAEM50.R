...
#   Date        :    18.08.2026
#   Purpose     :    reproduce the primary simulation results with 50% dropout
...
#
#   Output .txt files :  Data_and_Code/Data/simulation/SS-simulation-t50/SIM1-SIM5/
#
#   R Version   :    R-4.6.0
...
for (Rep in binrep:Repp) {
  for (ll in 1:length(N)) {
    n <- N[ll]
    if (n == 25) PATH1 <- file.path(PATH, "Data", "simulation", "SS-simulation-t50", "SIM1")
    if (n == 50) PATH1 <- file.path(PATH, "Data", "simulation", "SS-simulation-t50", "SIM2")
    if (n == 100) PATH1 <- file.path(PATH, "Data", "simulation", "SS-simulation-t50", "SIM3")
    if (n == 200) PATH1 <- file.path(PATH, "Data", "simulation", "SS-simulation-t50", "SIM4")
    if (n == 400) PATH1 <- file.path(PATH, "Data", "simulation", "SS-simulation-t50", "SIM5")
    set.seed(Rep + n*100)
