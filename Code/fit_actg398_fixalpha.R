################################################################################
#
#   Filename    :    fit_actg398_fixalpha.R
...
#
#   Output data files :  Data_and_Code/Data/fix_alpha/
...
per <- 500
M <- 10
SAVE_PATH <- file.path(PATH, "Data", "fix_alpha")
...
for (iii in 1:length(ll)) {
  ...
  alpha.tag <- paste0("_alpha", alpha[5])
  
  init.t.III.MNAR$alpha[5] = alpha[5]
  
  fit.t.III.ARp.MNAR <- try(tNLMM.miss.SAEM(Data, g = 1, init.para = init.t.III.MNAR, cor.type = c("ARp"), 
                                            M = 10, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, 
                                            per = per, mechanism = "MNAR", random.structure = "III"), silent = FALSE)
  
  if (inherits(fit.t.III.ARp.MNAR, "try-error")) next
  
  result <- rbind(result, c(kkk, alpha[5], fit.t.III.ARp.MNAR$model.inf$bic, sum(is.na(fit.t.III.ARp.MNAR$IM$out[2, ])), fit.t.III.ARp.MNAR$IM$out[, 6]))
  
  save.image(file.path(SAVE_PATH, paste0("fit.t.III.ARp.MNAR", alpha.tag, ".RData")))
}
