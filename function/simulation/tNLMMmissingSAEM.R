...
tNLMM.miss.SAEM = function(Data, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR',"MAR",'MCAR'))
{
  ...
  for (i in 1:N)
  {
    ...
    if (sum(i == y.na.ind) == 0) {
      ...
      logwden[i] <- mvtnorm::dmvt(c(Ytilde[idx1]), delta = c(TXtilde[idx1, ] %*% Beta), sigma = TLam[idx1, idx1], df = nu, log = TRUE) + log.drop.i
    } else {
      ...
      log.dt.ym <- mvtnorm::dmvt(as.matrix(y.hat.ll), delta = c(mu.mo[idx2, ]), sigma = as.matrix(Sig.mm.o[idx2, idx2]), df = nu + ni.o[y.na.ind][na.i], log = TRUE)
      logw <- rep(-Inf, M.LL.i)
      ...
      for (jj in 1:M.LL.i)
      {
        ...
        logden.i <- pmax(eta.i[keep.i], 0) + log1p(exp(-abs(eta.i[keep.i])))
        log.drop.i <- sum(R.i[keep.i] * eta.i[keep.i] - logden.i)
        log.fn <- mvtnorm::dmvt(y.hat.i, delta = c(TXtilde[idx1, ] %*% Beta), sigma = TLam[idx1, idx1], df = nu, log = TRUE) + log.drop.i
        logw[jj] <- log.fn - log.dt.ym[jj]
      }
      
      logw.finite <- logw[is.finite(logw)]
      if (length(logw.finite) == 0) stop("All importance weights are non-finite in initial t log-likelihood.")
  ...
  repeat
  {
    ...
    tau.Q <- matrix(0, nrow = mc.size, ncol = N)
    for (m in 1:mc.size) { for (i in 1:N) { if (i == 1) { idx1 <- 1:cumsum.ni[1]; idx2 <- 1:cumsum.q[1] } else { idx1 <- (cumsum.ni[i - 1] + 1):cumsum.ni[i]; idx2 <- (cumsum.q[i - 1] + 1):cumsum.q[i] }; e.i <- c(y.conv[m, idx1] - Xbeta[idx1]); Delta.i <- as.numeric(t(e.i) %*% TLam.inv[idx1, idx1] %*% e.i); tau.Q[m, i] <- as.numeric((nu + ni[i]) / (nu + Delta.i)); b.Q[m, idx2] <- DD %*% t(TZtilde[idx1, idx2]) %*% TLam.inv[idx1, idx1] %*% e.i } }
    ...
    #### Q-function based on SA-updated sufficient statistics
    Qloglik.MC <- 0
    for (m in 1:mc.size)
    {
      Qloglik.m <- 0
      ...
      for (i in 1:N)
      {
        ...
        b.i <- c(b.Q[m, idx2])
        tau.i <- tau.Q[m, i]
        log.y.i <- mvtnorm::dmvnorm(y.i, mean = c(X.i %*% Beta + Z.i %*% b.i), sigma = sigma / tau.i * C.i, log = TRUE)
        log.b.i <- mvtnorm::dmvnorm(b.i, mean = rep(0, q), sigma = DD / tau.i, log = TRUE)
        log.tau.i <- dgamma(tau.i, shape = nu / 2, rate = nu / 2, log = TRUE)
        R.i <- R[idx1]
        ...
        Qloglik.m <- Qloglik.m + log.y.i + log.b.i + log.tau.i + log.d.i
      }
    ...
    if (abs(diff.lnL) < tol || diff < tol || iter >= max.iter) { M.LL.i <- M.LL } else { M.LL.i <- M.LL / 100 }
    ...
    for (i in 1:N)
    {
      ...
      if (sum(i == y.na.ind) == 0) {
        ...
        logw.finite <- logw[is.finite(logw)]
        if (length(logw.finite) == 0) stop("All importance weights are non-finite in t log-likelihood.")
        max.logw <- max(logw.finite)
        logwden[i] <- max.logw + log(mean(exp(logw.finite - max.logw)))
      }
    }
