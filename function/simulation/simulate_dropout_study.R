# nonlinear function
mu.fn <- function(beta, x) {
  mu <- (beta[1]) / (1 + beta[2] * exp(beta[3] * x)) ### x=tjk###
  return(mu)
}

mu.fn.nlme <- Var1 ~ (beta1) / (1 + beta2 * exp(beta3 * Time))
nlme.deriv <- deriv(mu.fn.nlme,
                    namevec = c("beta1", "beta2", "beta3"),
                    function.arg = c("Time", "beta1", "beta2", "beta3")
)
...
dmu <- function(eta, x) {
  ...
  a1 <- 1 / (1 + eta[2] * exp(eta[3] * x))
  dmu1[, 1] <- a1
  dmu1[, 2] <- -(eta[1] * exp(eta[3] * x)) * (a1^2)
  dmu1[, 3] <- -(x * eta[1] * eta[2] * exp(eta[3] * x)) * (a1^2)
  return(dmu1)
}
...
MH.y.miss <- function(y.c, R, X, Beta, y.na.ind, TLam.i, V, psi, TO, TM) {
  ...
  for (j in y.na.ind)
  {
    ...
    # kk = log(runif(1)) > log.accept
    # cat(j, " subject,", " Accept: ", log(runif(1)) > log.accept,  "\n")
  }
  # return(list(y.c, idx.c))
  return(y.c)
}
...
# MH.y.miss1(y.gen[,,m-1], R, X, Beta, y.na.ind, TLam[[i]], V, psi, TO, TM)
MH.y.miss1 <- function(y.c, R, X, Beta, y.na.ind, cumsum.nj, TLam.i, V, psi, TO, TM) {
  ...
  for (j in y.na.ind)
  {
    ...
    # var.j = TM[[j]]%*% TLam.i[idx,idx] %*% t(TM[[j]]) - TM[[j]]%*% TLam.i[idx,idx] %*% t(TO[[j]]) %*% solve(TO[[j]]%*% TLam.i[idx,idx] %*% t(TO[[j]])) %*% TO[[j]]%*% TLam.i[idx,idx] %*% t(TM[[j]])
...
rmvslash <- function(n=1, mu, Sigma, slash.q=5) {
  if(slash.q <= 0) stop("slash.q must be positive.")
  Z <- mvtnorm::rmvnorm(n=n, mean=rep(0, length(mu)), sigma=Sigma)
  U <- runif(n)
  X <- sweep(Z, 1, U^(1 / slash.q), "/")
  X <- sweep(X, 2, mu, "+")
  return(X)
}

# Generate slash NLMM data:
gen.snlmm <- function(n, para, cor.type, si, q) {
  Beta <- para$Beta
  DD <- para$DD
  sigma <- para$sigma
  slash.q <- para$slash.q
  Phi <- para$Phi
  ...
  B[1, 1] <- 1
  Ti <- c(0, 2, 4, 8, 16 ,24)
  Ymat <- matrix(NA, nrow=n, ncol=si)
  
  for(j in 1:n) {
    bij[j, ] <- c(rmvslash(n=1, mu=rep(0, q), Sigma=DD, slash.q=slash.q))
    error.j <- c(rmvslash(n=1, mu=rep(0, si), Sigma=sigma * cor.fn(Phi, dim=si, type=cor.type[1], Ti=Ti, ga=1), slash.q=slash.q))
    Ymat[j, ] <- mu.fn(A %*% Beta + B %*% bij[j, ], Ti) + error.j
  }
  
  resp <- as.vector(t(Ymat[, 1:si]))
  sim.data <- data.frame(Var1=resp, Time=rep(c(0, 2, 4, 8, 16 ,24), n), Subject=rep(1:n, each=si), treat=rep(treat, each=si))
  return(list(Data=sim.data, b=bij, Ymat=Ymat))
}
...
add.MNAR.tNLMM <- function(Data, Ymat, b, si, alpha) {
  ...
  # Ytilde1 = NULL
  # for(j in 1:n){
  #   if(j == 1)idx1 = 1: cumsum.nj[1]
  #   else idx1 = (cumsum.nj[j-1]+1): cumsum.nj[j]
  #   Ytilde1 = c(Ytilde1, y[idx1] - MU[idx1] + TXtilde[idx1,] %*% Beta + as.matrix(TZtilde[idx1, j]) %*% b[j,])
  # }
  ...
  # Ymat[1,-1]
  # treat[1:si]
  Vi <- cbind(1, treat[1:si], c(0, Ytilde.mat[1, 1:(si - 1)]), Ytilde.mat[1, 1:si])
  eta <- Vi %*% alpha
  wi <- exp(eta) / (1 + exp(eta))
  p.na <- NULL
  p.na <- c(p.na, wi)
  for (j in 2:n)
  {
    ...
    Vi <- cbind(1, treat[idx1], c(0, Ytilde.mat[j, 1:(si - 1)]), Ytilde.mat[j, 1:si])
    eta <- Vi %*% alpha
    wi <- exp(eta) / (1 + exp(eta))
    p.na <- c(p.na, wi)
  }
...
yv.joint.log <- function(yi, Xbeta.i, Lam.i, Vi, R.i, alpha, n.i) {
  ...
  # dropout <- which(R.i == 1)[1]
  # if (!is.na(dropout)) pvi[(dropout+1):n.i, "Var1"] <- NA
...
# Vi=V[idx1,]; R.i = R[idx1]; n.i=ni[i]; mu.mo=mu.mo; Sig.mm.o=Sig.mm.o; na.idx.i=na.idx[[i]]; Data.miss.i = Data.miss[idx1,]
MH.y.miss2 <- function(yi, Xbeta.i, Lam.i, alpha, Vi, R.i, n.i, mu.mo, Sig.mm.o, na.idx.i, Data.miss.i, V.fun.i, Sig.mm.o.MC) {
  ...
  # if(log(runif(1)) > log.accept) print("True")
  # if(log(runif(1)) > log.accept & max(Data$week[idx1]) == 18) cat("yi.new =", yi.new, "yi.old = ",yi.old, "\n")
...
# y.samp; Data.miss; y.na.ind; Sig.mm.o.MC; Xbeta; TLam; tau; R; alpha; ni
# yi=y.samp[(m-1), idx1]; Xbeta.i=c(Xbeta)[idx1]; Lam.i=TLam[idx1, idx1];  tau.i = tau[i]; alpha=alpha
# Vi=V[idx1,]; R.i = R[idx1]; n.i=ni[i]; mu.mo=mu.mo; Sig.mm.o=Sig.mm.o; na.idx.i=na.idx[[i]]
...
prediction_ym <- function(Data, fm1.nlme, cor.type = "UNC") {
  ...
  # for(j in 1:N)
  # {
  #   if(j == 1)idx1 = 1: cumsum.ni.o[1]
  #   else idx1 = (cumsum.ni.o[j-1]+1): cumsum.ni.o[j]
  #   yo.cent1 = c(yo.cent1, Ytilde.o[idx1] - Xo[idx1,] %*% Beta)
  # }
  ...
  colnames(Data.a) <- c("int.", "treat.i", "fData.i", "bData.i")
  
  fm3 <- glm(R ~ treat.i + fData.i + bData.i, data = Data.a, family = binomial())
  summary(fm3)
