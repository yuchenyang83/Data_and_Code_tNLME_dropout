...
NLMM.miss.SAEM = function(Data, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR',"MAR",'MCAR'))
{
  begin = proc.time()[1]
  ...
  for (i in 1:N)
  {
    if (i == 1) { idx0 <- 1:cumsum.ni.o[1]; idx1 <- 1:cumsum.ni[1] } else { idx0 <- (cumsum.ni.o[i - 1] + 1):cumsum.ni.o[i]; idx1 <- (cumsum.ni[i - 1] + 1):cumsum.ni[i] }
    R.i <- R[idx1]
    ...
    if (sum(i == y.na.ind) == 0) {
      ...
      log.drop.i <- sum(R.i[keep.i] * eta.i[keep.i] - logden.i)
      logwden[i] <- mvtnorm::dmvnorm(c(Ytilde[idx1]), mean = c(TXtilde[idx1, ] %*% Beta), sigma = TLam[idx1, idx1], log = TRUE) + log.drop.i
    } else {
      na.i <- which(y.na.ind == i)
      if (na.i == 1) { idx2 <- 1:cumsum.na[1] } else { idx2 <- (cumsum.na[na.i - 1] + 1):cumsum.na[na.i] }
      y.hat.ll <- t(mu.mo[idx2, ] + t(mvtnorm::rmvnorm(M.LL, sigma = as.matrix(Sig.mm.o[idx2, idx2]))))
      log.dnorm.ym <- mvtnorm::dmvnorm(as.matrix(y.hat.ll), mean = c(mu.mo[idx2, ]), sigma = as.matrix(Sig.mm.o[idx2, idx2]), log = TRUE)
      logw <- rep(-Inf, M.LL.i)
      
      for (jj in 1:M.LL.i)
      {
        y.hat.i <- c(Ytilde[idx1][R.i == 0], y.hat.ll[jj, ])
        V.i <- V.fun.i(y.i = y.hat.i, Data.miss[idx1, ])
        eta.i <- c(V.i %*% alpha)
        keep.i <- if (sum(R.i) > 1) 1:(sum(R.i == 0) + 1) else 1:length(R.i)
        logden.i <- pmax(eta.i[keep.i], 0) + log1p(exp(-abs(eta.i[keep.i])))
        log.drop.i <- sum(R.i[keep.i] * eta.i[keep.i] - logden.i)
        log.fn <- mvtnorm::dmvnorm(y.hat.i, mean = c(TXtilde[idx1, ] %*% Beta), sigma = TLam[idx1, idx1], log = TRUE) + log.drop.i
        logw[jj] <- log.fn - log.dnorm.ym[jj]
      }
      
      logw.finite <- logw[is.finite(logw)]
      if (length(logw.finite) == 0) stop("All importance weights are non-finite in initial normal log-likelihood.")
      max.logw <- max(logw.finite)
      logwden[i] <- max.logw + log(mean(exp(logw.finite - max.logw)))
    }
  }
  
  loglik.old <- iter.lnL <- sum(logwden)
  ...
  repeat
  {
    ...
    for(m in 2: M)
    {
      for(i in y.na.ind)
      {
        ...
        y.samp[m, idx1] = MH.y.miss2(yi=y.samp[(m-1), idx1], Xbeta.i=c(Xbeta)[idx1], Lam.i=TLam[idx1, idx1], alpha=alpha, 
    ...
    for (m in 1:mc.size) { for (i in 1:N) { if (i == 1) { idx1 <- 1:cumsum.ni[1]; idx2 <- 1:cumsum.q[1] } else { idx1 <- (cumsum.ni[i - 1] + 1):cumsum.ni[i]; idx2 <- (cumsum.q[i - 1] + 1):cumsum.q[i] }; e.i <- c(y.conv[m, idx1] - Xbeta[idx1]); b.Q[m, idx2] <- DD %*% t(TZtilde[idx1, idx2]) %*% TLam.inv[idx1, idx1] %*% e.i } }
    ...
    for(m in 1:mc.size)
    {
      ...
      for(i in 1:N)
      {
        ...
        wij.SA[idx1] = wij.SA[idx1] + plogis(c(as.matrix(V[idx1, ]) %*% alpha))
      }
    }
    ...
    if(cor.type == "CAR1" | cor.type == 'ARp'){
      ga = 1
      Phi = optim(par = Phi, fn = phiga.fn, method = "L-BFGS-B", lower = -1+1e-6, upper = 1-1e-6, 
                  sigma=sigma, E.hat1=E.hat, cumsum.ni=cumsum.ni, N=N, ni=ni, Data=Data, cor.type = cor.type[1])$par
    }
    if(cor.type == 'CS'){
      ga = 1
      Phi = optim(par = Phi, fn = phiga.fn, method = "L-BFGS-B", lower = 1e-6, upper = 1-1e-6, 
                  sigma=sigma, E.hat1=E.hat, cumsum.ni=cumsum.ni, N=N, ni=ni, Data=Data, cor.type = cor.type[1])$par
    }
    if(cor.type == 'DEC'){
      par.DEC = optim(par = c(Phi, ga), fn = phiga.fn, method = "L-BFGS-B", lower = c(1e-6, 1e-6), upper = c(1-1e-6, Inf), 
                      sigma=sigma, E.hat1=E.hat, cumsum.ni=cumsum.ni, N=N, ni=ni, Data=Data, cor.type = cor.type[1])$par
      Phi = par.DEC[1]; ga = par.DEC[2]
    }
    if(cor.type == 'BAND1'){
      ga = 1
      Phi = optim(par = Phi, fn = phiga.fn, method = "L-BFGS-B", lower = -1/2, upper = 1/2, 
                  sigma=sigma, E.hat1=E.hat, cumsum.ni=cumsum.ni, N=N, ni=ni, Data=Data, cor.type = cor.type[1])$par
    }
    ...
    for (m in 1:mc.size)
    {
      ...
      for (i in 1:N)
      {
        ...
        keep.i <- if (sum(R.i) > 1) 1:(sum(R.i == 0) + 1) else 1:length(R.i)
        log.p.i <- ifelse(eta.i[keep.i] >= 0, -log1p(exp(-eta.i[keep.i])), eta.i[keep.i] - log1p(exp(eta.i[keep.i])))
        log.1mp.i <- ifelse(eta.i[keep.i] >= 0, -eta.i[keep.i] - log1p(exp(-eta.i[keep.i])), -log1p(exp(eta.i[keep.i])))
        log.d.i <- sum(R.i[keep.i] * log.p.i + (1 - R.i[keep.i]) * log.1mp.i)
        Qloglik.m <- Qloglik.m + log.y.i + log.b.i + log.d.i
      }
      Qloglik.MC <- Qloglik.MC + Qloglik.m
    }
    Qloglik.MC <- Qloglik.MC / mc.size
    Qloglik <- if (is.na(Qloglik)) Qloglik.MC else Qloglik + gamma.h[iter] * (Qloglik.MC - Qloglik)
    ...
    if (diff < tol || iter >= max.iter) { M.LL.i <- M.LL } else { M.LL.i <- M.LL / 100 }
    ...
    for (i in 1:N)
    {
      ...
      if (sum(i == y.na.ind) == 0) {
        ...
        logw.finite <- logw[is.finite(logw)]
        if (length(logw.finite) == 0) stop("All importance weights are non-finite in normal log-likelihood.")
        max.logw <- max(logw.finite)
        logwden[i] <- max.logw + log(mean(exp(logw.finite - max.logw)))
      }
    }
    
    loglik.new <- sum(logwden)
    
    #### Fisher information
    para.est = list(Beta = Beta, sigma = sigma, D = DD, Phi = Phi, alpha=alpha, b=b.hat)
    IM = I.lmm.missing(para.est, cor.type=cor.type, X=TXtilde, Z = TZtilde, N=N, cumsum.ni=cumsum.ni, cumsum.q=cumsum.q, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
    ...
    diff.lnL = abs(loglik.new-loglik.old)
    theta.new = c(Beta, DD[vechD], sigma, Phi, alpha)
    Tpara = rbind(Tpara, theta.new)
    diff <- sqrt(sum((theta.new - theta.old)^2))
    if(iter%%per == 0) cat('iter = ', iter, ',\t obs.loglik = ', loglik.new, ',\t Q.loglik = ', Qloglik, ',\t theta.diff = ', diff, ',\t MSE of y = ', sum((y.hat[na.ind] - Data$y.c[na.ind])^2) / length(na.ind), sep = ' ', '\n')
    # if(iter%%per == 0) cat('iter = ', iter, ',\t obs.loglik = ', loglik.new, ',\t theta.diff = ', diff, ',\t MSE of y = ', sum((y.hat[na.ind] - Data$y.c[na.ind])^2) / length(na.ind), sep = ' ', '\n')
    if(iter%%per == 0) cat('sigma = ', sigma, "DD = ", DD[vechD], "phi = ", Phi, sep = ' ', '\n')
  ...
  # IM2 = I.lmm.missing(para.est, cor.type=cor.type, X=TXtilde, Z = TZtilde, N=N, cumsum.ni=cumsum.ni, cumsum.q=cumsum.q, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
  #                    y.conv=y.conv, mechanism=mechanism, dropout.idx=dropout.idx, R=R, n=n, Data.miss=Data.miss)
  # 
  # 
  cat(rep("=", 50), sep = "", "\n")
  ...
  cat("aic =", aic, "\n")
  cat("bic =", bic, "\n")
  cat("MSE of y =", sum((y.hat[na.ind] - Data$y.c[na.ind])^2) / length(na.ind), "\n")
  cat(paste(rep("=", 50), sep = "", collapse = ""), "\n")
  ...
  return(list(model.inf = model.inf, para.est = para.est, iter=iter, y.c=y.hat, IM=IM, 
              MSE.y = sum((y.hat[na.ind] - Data$y.c[na.ind])^2) / length(na.ind), MAE.y = sum(abs(y.hat[na.ind] - Data$y.c[na.ind])) / length(na.ind), MAPE.y = sum(abs((y.hat[na.ind] - Data$y.c[na.ind]) / Data$y.c[na.ind])) / length(na.ind),
              na.ind = na.ind, Tpara=Tpara, Taccept.rate = Taccept.rate))
}


I.lmm.missing = function(para.est, cor.type=cor.type, X, Z, N, cumsum.ni, cumsum.q, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, Data.miss)
{
  Beta = para.est$Beta
  DD = para.est$D
